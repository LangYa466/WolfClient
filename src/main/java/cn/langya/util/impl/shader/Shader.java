...
public abstract class Shader extends Util {
    private int program;
    ...
    public Shader(final String fragmentShader) {
        ...
        try {
            final InputStream vertexStream = getClass().getResourceAsStream("/assets/minecraft/wolfclient/shader/vertex.vert");
            vertexShaderID = createShader(IOUtils.toString(vertexStream), ARBVertexShader.GL_VERTEX_SHADER_ARB);
            IOUtils.closeQuietly(vertexStream);

            final InputStream fragmentStream = getClass().getResourceAsStream("/assets/minecraft/wolfclient/shader/fragment/" + fragmentShader);
       ...
       ClientUtil.loginfo("[着色器] 成功加载: " + fragmentShader);
    }
    ...
    private int createShader(String shaderSource, int shaderType) {
        ...
        try {
            ...
            if (ARBShaderObjects.glGetObjectParameteriARB(shader, ARBShaderObjects.GL_OBJECT_COMPILE_STATUS_ARB) == GL11.GL_FALSE)
                throw new RuntimeException("创建着色器失败 : " + getLogInfo(shader));
